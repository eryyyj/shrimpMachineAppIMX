#!/bin/bash

# 1. Enable the virtual camera module (Requires sudo)
sudo modprobe v4l2loopback video_nr=10 card_label="ShrimpSenseCam" exclusive_caps=1

# 2. Run your WORKING camera command in the background
# The '&' at the end is crucial so the script can move on to the next line
rpicam-vid -t 0 --width 1280 --height 720 --framerate 30 --codec yuv420 --inline -n -o - | \
ffmpeg -f rawvideo -vcodec rawvideo -pixel_format yuv420p -video_size 1280x720 -framerate 30 -i - \
-f v4l2 -vcodec rawvideo -pix_fmt yuyv422 /dev/video10 &

# 3. Wait a few seconds for the camera stream to stabilize
sleep 3

# 4. Set Environment Variables
export MONGO_URI="mongodb+srv://qajgvalencia:BUxIhYb4nDlfH4DV@cluster0.h07iggq.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
export MONGO_DB="test"
export QT_QPA_PLATFORM="Wayland"
export QT_AUTO_SCREEN_SCALE_FACTOR=0
export QT_SCALE_FACTOR=1
export QT_FONT_DPI=96

# 5. Move to project folder and run the app
cd /home/hiponpd/Documents/GitHub/ShrimpMachineApp
/home/hiponpd/Documents/GitHub/ShrimpMachineApp/venv/bin/python3 app.py

# 6. Cleanup: Stop the camera bridge when you close the app
pkill -f rpicam-vid
pkill -f ffmpeg